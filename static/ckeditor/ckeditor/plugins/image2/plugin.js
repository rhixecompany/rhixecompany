...
(function () {
      ...
      if (this.deflated)
        ((this.widget = a.widgets.initOn(this.element, "image", this.widget.data)),
          this.widget.inline &&
            ...
            ((d = c.createElement(a.activeEnterMode == CKEDITOR.ENTER_P ? "p" : "div")),
        ...
        if (d) {
          ...
          "center" == e
            ? (b ? c.setStyle("text-align", "center") : c.removeStyle("text-align"), c.removeStyle("float"))
            : ("none" == e ? c.removeStyle("float") : c.setStyle("float", e), c.removeStyle("text-align"));
      }
    }
    ...
    return {
      ...
      init: function () {
        ...
        e.align ||
          ...
          f
            ? (c.hasClass(f[0]) ? (e.align = "left") : c.hasClass(f[2]) && (e.align = "right"),
              e.align ? c.removeClass(f[n[e.align]]) : (e.align = "none"))
            : ((e.align = c.getStyle("float") || "none"), c.removeStyle("float")));
        if (
          a.plugins.link &&
          this.parts.link &&
          ((e.link = CKEDITOR.plugins.link.parseLinkAttributes(a, this.parts.link)),
          (c = e.link.advanced) && c.advCSSClasses)
        )
          c.advCSSClasses = CKEDITOR.tools.trim(c.advCSSClasses.replace(/cke_\S+/, ""));
      ...
      getClasses: (function () {
        ...
        return function () {
          var b = this.repository.parseElementClasses(k(this).getAttribute("class")),
            e;
    ...
    return function (a) {
      ...
      d &&
        ...
        "center" == d && "figure" == a.name
          ...
          : d in { left: 1, right: 1 } && (b ? f.addClass(b[n[d]]) : (c["float"] = d)),
        !b && !CKEDITOR.tools.isEmpty(c) && (g.style = CKEDITOR.tools.writeCssText(c)));
      return a;
    };
  }
  function l(a) {
    ...
    return function (g) {
      ...
      return (
        e ? g.hasClass(e[1]) : "center" == CKEDITOR.tools.parseCssText(g.attributes.style || "", !0)["text-align"]
      )
        ? !0
        : !1;
    };
    ...
    return function (f) {
      ...
      if (g) {
        ...
        g.on("refresh", function (b) {
          ...
          c &&
            (void 0 === e && (e = a.filter.checkFeature(a.widgets.registered.image.features.align)),
            e
              ? this.setState(
                  c.data.align == f
  ...
  function J(a) {
    a.plugins.link &&
      ...
      a.getCommand("unlink").on("refresh", function (b) {
        ...
        e &&
          (this.setState(
            e.data.link || e.wrapper.getAscendant("a") ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED,
          ),
          b.cancel());
      }));
  }
  ...
  var z = '<img alt="" src="" />',
    K = new CKEDITOR.template(
      '<figure class="{captionedClass}">' + z + "<figcaption>{captionPlaceholder}</figcaption></figure>",
    ),
    n = { left: 0, center: 1, right: 2 },
    G = /^\s*(\d+\%)\s*$/i;
  CKEDITOR.plugins.add("image2", {
    ...
    init: function (a) {
      ...
      a.contextMenu &&
        (a.addMenuGroup("image", 10), a.addMenuItem("image", { label: e.menu, command: "image", group: "image" }));
        ...
        i = {
          align: function (c, d, e) {
            ...
            if (c.changed.align) {
              if (
                ...
                !c.changed.hasCaption && "center" == d && "center" != e)
              )
                (c.deflate(), (d = f.findOne("a,img")), d.replace(f), (c.element = d));
            } else
              "center" == e && c.changed.hasCaption && !c.newData.hasCaption && (c.deflate(), (c.element = b(a, f)));
            !g && f.is("figure") && ("center" == e ? f.setStyle("display", "inline-block") : f.removeStyle("display"));
          },
          hasCaption: function (b, c, g) {
            b.changed.hasCaption &&
              ((c = b.element.is({ img: 1, a: 1 }) ? b.element : b.element.findOne("a,img")),
              b.deflate(),
              g
                ? ((g = CKEDITOR.dom.element.createFromHtml(
                    K.output({ captionedClass: d, captionPlaceholder: a.lang.image2.captionPlaceholder }),
                    f,
                  )),
                  e(g, b.element),
          ...
          link: function (b, c, d) {
            if (b.changed.link) {
              ...
              d
                ? (c ||
                    ((i = f.createElement("a", { attributes: { href: b.newData.link.url } })), i.replace(e), e.move(i)),
                  (d = CKEDITOR.plugins.link.getLinkAttributes(a, d)),
                  CKEDITOR.tools.isEmpty(d.set) || (i || g).setAttributes(d.set),
                  d.removed.length && (i || g).removeAttributes(d.removed))
                : ((d = g.findOne("img")), d.replace(g), (i = d));
              h && (b.element = i);
            }
          },
        };
