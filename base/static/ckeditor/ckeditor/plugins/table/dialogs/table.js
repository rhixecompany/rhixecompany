      ...
      onOk: function () {
        ...
        if (c.info) {
          ...
          if (!this._.selectedElement)
            for (
              var h = b.append(p("tbody")), f = parseInt(c.txtRows, 10) || 0, l = parseInt(c.txtCols, 10) || 0, k = 0;
              k < f;
              k++
            )
          ...
          if (null !== b.$.tHead && "row" != f && "both" != f) {
            ...
            for (h = b.getElementsByTag("tbody").getItem(0); 0 < g.getChildCount();) {
              ...
              for (k = 0; k < l.getChildCount(); k++)
                ((m = l.getChild(k)),
                  m.type == CKEDITOR.NODE_ELEMENT && (m.renameNode("td"), m.removeAttribute("scope")));
              h.append(l, !0);
            }
            g.remove();
          }
            ...
            for (k = 0; k < b.$.rows.length; k++)
              ((g = new CKEDITOR.dom.element(b.$.rows[k])),
                "tbody" == g.getParent().getName() &&
                  ((m = new CKEDITOR.dom.element(g.$.cells[0])), m.renameNode("td"), m.removeAttribute("scope")));
                    ...
                    if (this.isEnabled()) {
                      ...
                      if (b)
                        (0 < c.count()
                          ...
                          : ((c = new CKEDITOR.dom.element("caption", a.document)), e.append(c, !0)),
