...
Notification.prototype = {
  ...
  _createElement: function () {
    ...
    c = CKEDITOR.dom.element.createFromHtml(
      '<a class="cke_notification_close" href="javascript:void(0)" title="' +
        d +
        '" role="button" tabindex="-1"><span class="cke_label">X</span></a>',
    );
    a.append(c);
    c.on("click", function () {
      b.editor.focus();
...
Area.prototype = {
  ...
  _layout: function () {
    ...
    var m = j.getScrollPosition(),
      n = j.getViewPaneSize(),
      o = CKEDITOR.document.getBody(),
      p = o.getDocumentPosition(),
      i = CKEDITOR.tools.cssLength;
    if (!f || !h)
      ((j = this.element.getChild(0)),
        (f = this._notificationWidth = j.getClientRect().width),
        (h = this._notificationMargin =
          parseInt(j.getComputedStyle("margin-left"), 10) + parseInt(j.getComputedStyle("margin-right"), 10)));
    c.isVisible() && g.bottom > d.top && g.bottom < d.bottom - k.height
      ? a.setStyles({ position: "fixed", top: i(g.bottom) })
      : 0 < d.top
        ? a.setStyles({ position: "absolute", top: i(e.y) })
        : e.y + d.height - k.height > m.y
    ...
    d.width < f + h
      ...
      : e.x + f + h > m.x + n.width
        ? a.setStyle("left", i(l))
        : e.x + d.width / 2 + f / 2 + h > m.x + n.width
          ? a.setStyle("left", i(l - e.x + m.x + n.width - f - h))
          : 0 > d.left + d.width - f - h
            ? b()
            : 0 > d.left + d.width / 2 - f / 2
              ? a.setStyle("left", i(l - e.x + m.x))
              : a.setStyle("left", i(l + d.width / 2 - f / 2 - h / 2));
  },
};
