  ...
  CKEDITOR.plugins.add("mathjax", {
    ...
    init: function (b) {
      ...
      !b.config.mathJaxLib &&
        ...
        window.console.log &&
        window.console.log(
          "Error: config.mathJaxLib property is not set. For more information visit: ",
          "http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-mathJaxLib",
        );
      b.widgets.add("mathjax", {
        ...
        init: function () {
          ...
          if (!a || a.type != CKEDITOR.NODE_ELEMENT || !a.is("iframe"))
      ...
      b.on("contentPreview", function (a) {
        a.data.dataValue = a.data.dataValue.replace(
          /<\/head>/,
          '<script src="' + CKEDITOR.getUrl(b.config.mathJaxLib) + '"><\/script></head>',
        );
      });
    ...
    CKEDITOR.env.ie && 8 == CKEDITOR.env.version
      ? function (b, c) {
          b.getFrameDocument().write(
            '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="padding:0;margin:0;background:transparent;overflow:hidden"><span style="white-space:nowrap;" id="tex"></span></body></html>',
          );
          return {
            setValue: function (a) {
      ...
      : function (b, c) {
          function a() {
            ...
            f.getById("preview") ||
              ...
              f.write(
                '<!DOCTYPE html><html><head><meta charset="utf-8"><script type="text/x-mathjax-config">MathJax.Hub.Config( {showMathMenu: false,messageStyle: "none"} );function getCKE() {if ( typeof window.parent.CKEDITOR == \'object\' ) {return window.parent.CKEDITOR;} else {return window.parent.parent.CKEDITOR;}}function update() {MathJax.Hub.Queue([ \'Typeset\', MathJax.Hub, this.buffer ],function() {getCKE().tools.callFunction( ' +
                  l +
                  " );});}MathJax.Hub.Queue( function() {getCKE().tools.callFunction(" +
                  m +
                  ');} );<\/script><script src="' +
                  c.config.mathJaxLib +
                  '"><\/script></head><body style="padding:0;margin:0;background:transparent;overflow:hidden"><span id="preview"></span><span id="buffer" style="display:none"></span></body></html>',
              ));
          }
          function e() {
            k = !0;
          ...
          var d,
            ...
            l = CKEDITOR.tools.addFunction(function () {
              ...
              b.setStyles({ height: a + "px", width: j + "px" });
              c.fire("unlockSnapshot");
