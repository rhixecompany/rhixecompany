    ...
    if (b && b.data("cke-saved-href")) {
      ...
      if (e) {
        var f = "function" == typeof e.defaults ? e.defaults() : e.defaults,
          ...
          h,
          k = a.widgets.wrapElement(f, e.name),
            ...
            h.loadContent(b, {
              ...
              callback: function () {
                ...
                if (b) {
                  ...
                  e.setEndAfter(b);
                  f.insertElement(k, e);
                  f.contains(i) && f.contains(h) ? c.selectBookmarks([j]) : (i.remove(), h.remove());
        ...
        window.console &&
          window.console.log(
            "[CKEDITOR.plugins.autoEmbed] Incorrect config.autoEmbed_widget value. No widget definition found.",
          );
    }
  }
  var n = /^<a[^>]+href="([^"]+)"[^>]*>([^<]+)<\/a>$/i;
        ...
        function (c) {
          if (c.data.dataTransfer.getTransferType(a) == CKEDITOR.DATA_TRANSFER_INTERNAL) b = 0;
            ...
            if ((b = null != d && decodeURI(d[1]) == decodeURI(d[2])))
              c.data.dataValue = '<a data-cke-autoembed="' + ++g + '"' + c.data.dataValue.substr(2);
          }
        },
        null,
        null,
