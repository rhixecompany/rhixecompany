...
var jush = {
  sql_function:
    "mysql_db_query|mysql_query|mysql_unbuffered_query|mysqli_master_query|mysqli_multi_query|mysqli_query|mysqli_real_query|mysqli_rpl_query_type|mysqli_send_query|mysqli_stmt_prepare",
  ...
  highlight: function (language, text) {
    ...
    return (
      '<span class="jush">' +
      this.highlight_states([language], text.replace(/\r\n?/g, "\n"), language != "htm" && language != "tag")[0] +
      "</span>"
    );
  },
    ...
    for (var i = 0; i < pre.length; i++) {
      ...
      if (match) {
        var s = this.highlight(
          match[3] ? match[3] : "htm",
          this.html_entity_decode(pre[i].innerHTML.replace(/<br(\s+[^>]*)?>/gi, "\n").replace(/<[^>]*>/g, "")),
        )
          .replace(/\t/g, tab.length ? tab : "\t")
          .replace(/(^|\n| ) /g, "$1&nbsp;");
  ...
  highlight_states: function (states, text, in_php, escape) {
    ...
    loop: while (start < text.length && (match = regexps[state].exec(text))) {
      for (var key in tr[state]) {
        var m;
        if ((m = tr[state][key].exec(match[0])) && !m[0].index && m[0].length == match[0].length) {
          ...
          if (
            (state == "att_quo" || state == "att_apo" || state == "att_val") &&
            (prev_state == "att_js" || prev_state == "att_css" || /^\s*javascript:/i.test(s))
          ) {
            // javascript: - easy but without own state //! should be checked only in %URI;
            child_states.unshift(prev_state == "att_css" ? "css_pro" : "js");
            s_states = this.highlight_states(
              child_states,
              this.html_entity_decode(s),
              true,
              state == "att_apo"
                ? this.htmlspecialchars_apo
                : state == "att_quo"
                  ? this.htmlspecialchars_quo
                  : this.htmlspecialchars_quo_apo,
            );
            ...
            (state == "php_quo" || state == "php_apo") &&
            (prev_state == "php_sql" ||
              prev_state == "php_sqlite" ||
              prev_state == "php_pgsql" ||
              prev_state == "php_phpini")
          ) {
            ...
            s_states = this.highlight_states(
              ...
              this.stripslashes(s),
              true,
              state == "php_apo" ? this.addslashes_apo : this.addslashes_quo,
            );
            ...
            ((state == "php_quo" || state == "php_apo") && prev_state == "php_echo") ||
            (state == "php_eot2" && states[states.length - 3] == "php_echo")
          ) {
            var i;
            for (i = states.length; i--;) {
              prev_state = states[i];
              if (
                prev_state.substring(0, 3) != "php" &&
                prev_state != "att_quo" &&
                prev_state != "att_apo" &&
                prev_state != "att_val"
              ) {
            ...
            var f =
              state == "php_eot2" ? this.addslashes : state == "php_apo" ? this.addslashes_apo : this.addslashes_quo;
            ...
            if (prev_state == "att_js" || prev_state == "att_css") {
              ...
              s_states = [
                escape ? escape(s) : s,
                isNaN(+key) ||
                !/^(att_js|att_css|css_js|js_write|php_sql|php_sqlite|php_pgsql|php_echo|php_phpini)$/.test(state) ||
                /^(js_write|php_echo|php_sql|php_sqlite|php_pgsql|php_phpini|css_js)$/.test(prev_state)
                  ? child_states
                  : [],
              ];
            }
            ...
            s_states = [
              ...
              /^(js_write|php_echo|php_sql|php_sqlite|php_pgsql|php_phpini|css_js)$/.test(prev_state)
                ? child_states
                : [],
            ]; // reset child states when escaping construct
          }
          s = s_states[0];
          ...
          s =
            m.length < 3
              ? s
                ? '<span class="jush-op">' + this.htmlspecialchars(escape ? escape(s) : s) + "</span>"
                : ""
              : (m[1]
                  ? '<span class="jush-op">' + this.htmlspecialchars(escape ? escape(m[1]) : m[1]) + "</span>"
                  : "") +
                this.htmlspecialchars(escape ? escape(m[2]) : m[2]) +
                (m[3]
                  ? '<span class="jush-op">' + this.htmlspecialchars(escape ? escape(m[3]) : m[3]) + "</span>"
                  : "");
          if (isNaN(+key)) {
            if (this.links && this.links[key] && m[2]) {
              ...
              var att_tag = att_mapping[link + "-" + this.last_tag]
              ...
              if (k_link) {
                ...
                s +=
                  '<a href="' +
                  this.urls[key].replace(/\$key/, k_link).replace(/\$val/, link) +
                  '">' +
                  this.htmlspecialchars(escape ? escape(m[2]) : m[2]) +
                  "</a>";
                s += m[3]
  ...
  html_entity_decode: function (string) {
    return string
      ...
      .replace(/&gt;/g, ">")
      .replace(/&quot;/g, '"')
      .replace(/&#(?:([0-9]+)|x([0-9a-f]+));/gi, function (str, p1, p2) {
        //! named entities
...
jush.urls = {
  ...
  phpini: [
    "http://www.php.net/$key",
    "features.safe-mode#ini.$1",
    "ini.core#ini.$1",
    "apache.configuration#ini.$1",
    "apc.configuration#ini.$1",
    "apd.configuration#ini.$1",
    "bc.configuration#ini.$1",
    "com.configuration#ini.$1",
    "datetime.configuration#ini.$1",
    "dbx.configuration#ini.$1",
    "errorfunc.configuration#ini.$1",
    "exif.configuration#ini.$1",
    "expect.configuration#ini.$1",
    "filesystem.configuration#ini.$1",
    "ibase.configuration#ini.$1",
    "ibm-db2.configuration#ini.$1",
    "ifx.configuration#ini.$1",
    "image.configuration#ini.image.jpeg-ignore-warning",
    "info.configuration#ini.$1",
    "mail.configuration#ini.$1",
    "mail.configuration#ini.smtp",
    "maxdb.configuration#ini.$1",
    "mbstring.configuration#ini.$1",
    "mime-magic.configuration#ini.$1",
    "misc.configuration#ini.$1",
    "misc.configuration#ini.syntax-highlighting",
    "msql.configuration#ini.$1",
    "mysql.configuration#ini.$1",
    "mysqli.configuration#ini.$1",
    "network.configuration#ini.$1",
    "nsapi.configuration#ini.$1",
    "oci8.configuration#ini.$1",
    "outcontrol.configuration#ini.$1",
    "pcre.configuration#ini.$1",
    "pdo-odbc.configuration#ini.$1",
    "pgsql.configuration#ini.$1",
    "runkit.configuration#ini.$1",
    "session.configuration#ini.$1",
    "soap.configuration#ini.$1",
    "sqlite.configuration#ini.$1",
    "sybase.configuration#ini.$1",
    "tidy.configuration#ini.$1",
    "unicode.configuration#ini.$1",
    "odbc.configuration#ini.$1",
    "zlib.configuration#ini.$1",
  ],
  py: [
    "http://docs.python.org/lib/$key.html",
    "browser-controllers",
    "built-in-funcs",
    "csv-contents",
  ...
  sql: [
    "http://dev.mysql.com/doc/mysql/en/$key",
    "$1.html",
    "commit.html",
    "savepoints.html",
    "lock-tables.html",
    "numeric-type-overview.html",
    "date-and-time-type-overview.html",
    "string-type-overview.html",
    "comparison-operators.html#operator_$1",
    "comparison-operators.html#function_$1",
    "any-in-some-subqueries.html",
    "row-subqueries.html",
    "group-by-modifiers.html",
    "string-comparison-functions.html#operator_$1",
    "logical-operators.html#operator_$1",
    "control-flow-functions.html#operator_$1",
    "arithmetic-functions.html#operator_$1",
    "cast-functions.html#operator_$1",
    "", // keywords without link
    "comparison-operators.html#function_$1",
    "control-flow-functions.html#function_$1",
    "string-functions.html#function_$1",
    "string-comparison-functions.html#function_$1",
    "mathematical-functions.html#function_$1",
    "date-and-time-functions.html#function_$1",
    "cast-functions.html#function_$1",
    "xml-functions.html#function_$1",
    "bit-functions.html#function_$1",
    "encryption-functions.html#function_$1",
    "information-functions.html#function_$1",
    "miscellaneous-functions.html#function_$1",
    "group-by-functions.html#function_$1",
    "fulltext-search.html#$1",
  ],
  sqlite: [
    "http://www.sqlite.org/$key",
    "lang_$1.html",
    "pragma.html",
    ...
    "lang_expr.html#$1",
    "lang_expr.html#corefunctions",
    "cvstrac/wiki?p=DateAndTimeFunctions#$1",
    "lang_expr.html#aggregatefunctions",
  ],
  pgsql: [
    "http://www.postgresql.org/docs/8.2/static/$key",
    "sql-$1.html",
    "sql-$1.html",
  ...
  cnf: [
    "http://httpd.apache.org/docs/2.2/mod/$key.html#$1",
    "beos",
    "core",
    "mod_actions",
    "mod_alias",
  ...
  js: [
    "http://developer.mozilla.org/En/$key",
...
jush.links = {
  ...
  att_js: {
    "interact/scripts":
      /^(onblur|onchange|onclick|ondblclick|onfocus|onkeydown|onkeypress|onkeyup|onload|onload|onmousedown|onmousemove|onmouseout|onmouseover|onmouseup|onreset|onselect|onsubmit|onunload|onunload)$/i,
  },
  att: {
    "interact/forms":
    ...
    "present/frames":
      /^(cols-FRAMESET|frameborder|height-IFRAME|longdesc-FRAME|marginheight|marginwidth|name-FRAME|noresize|rows-FRAMESET|scrolling|src-FRAME|target|width-IFRAME)$/i,
    "present/graphics":
      /^(align-HR|align|bgcolor|bgcolor|bgcolor|bgcolor|clear|color-FONT|face-FONT|noshade|size-HR|size-FONT|size-BASEFONT|width-HR)$/i,
    "present/styles": /^(media|media|type-STYLE)$/i,
    "struct/dirlang": /^(dir|dir-BDO|lang)$/i,
    "struct/global":
      /^(alink|background|class|content|http-equiv|id|link|name-META|profile|scheme|text|title|version|vlink)$/i,
    ...
    "struct/objects":
      /^(align-IMG|alt|alt|alt|archive-APPLET|archive-OBJECT|border-IMG|classid|code|codebase-OBJECT|codebase-APPLET|codetype|coords|coords|data|declare|height-IMG|height-APPLET|hspace|ismap|longdesc-IMG|name-APPLET|name-IMG|name-MAP|name-PARAM|nohref|object|shape|shape|src-IMG|standby|type-OBJECT|type-PARAM|usemap|value-PARAM|valuetype|vspace|width-IMG|width-APPLET)$/i,
    "struct/tables":
      /^(abbr|align-CAPTION|align-TABLE|align-TD|axis|border-TABLE|cellpadding|cellspacing|char|charoff|colspan|frame|headers|height-TH|nowrap|rowspan|rules|scope|span-COL|span-COLGROUP|summary|valign|width-TABLE|width-TH|width-COL|width-COLGROUP)$/i,
    "struct/text": /^(cite-Q|cite-INS|datetime|width-PRE)$/i,
  },
  css_val: {
    aural:
      /^(azimuth|cue-after|cue-before|cue|elevation|pause-after|pause-before|pause|pitch-range|pitch|play-during|richness|speak-header|speak-numeral|speak-punctuation|speak|speech-rate|stress|voice-family|volume)$/i,
    box: /^(border(?:-top|-right|-bottom|-left)?(?:-color|-style|-width)?|margin(?:-top|-right|-bottom|-left)?|padding(?:-top|-right|-bottom|-left)?)$/i,
    colors:
      /^(background-attachment|background-color|background-image|background-position|background-repeat|background|color)$/i,
    fonts: /^(font-family|font-size|font-style|font-variant|font-weight|font)$/i,
    generate:
      /^(content|counter-increment|counter-reset|list-style-image|list-style-position|list-style-type|list-style|quotes)$/i,
